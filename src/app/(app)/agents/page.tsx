...
export default function AgentsPage() {
  ...
  return (
    <div className="flex flex-col md:flex-row h-[calc(100vh-4rem-env(safe-area-inset-top)-env(safe-area-inset-bottom))] p-2 md:p-4 gap-2 md:gap-4 max-w-7xl mx-auto animate-in fade-in duration-300 overflow-hidden">
      {/* Sidebar: Agents List - Hidden on mobile, shown as tabs or drawer */}
      <Card className="hidden md:flex w-64 lg:w-80 flex-col shrink-0 border-border/50 shadow-sm overflow-hidden bg-background/50 backdrop-blur-sm">
        <div className="p-4 border-b border-border/50 flex items-center justify-between bg-muted/20">
          <div>
            <h2 className="text-base md:text-lg font-bold tracking-tight">智能代理列表</h2>
      ...
      <div className="flex-1 min-w-0 flex flex-col overflow-hidden">
        {/* Mobile Agent Selector - Styled Dropdown */}
        <div className="md:hidden p-3 border-b bg-muted/10">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" size="sm" className="w-full h-9 rounded-xl bg-background/80 border-border/50 justify-between font-medium text-sm">
                <span className="flex items-center gap-2">
                  ...
                  {selectedAgent?.identity?.name || selectedAgent?.name || selectedAgent?.id || "选择代理"}
                </span>
            ...
            <DropdownMenuContent className="w-[calc(100vw-32px)] max-h-[60vh] overflow-y-auto">
              {agents.map((agent: any) => (
                <DropdownMenuItem
                  key={agent.id}
                  onClick={() => setSelectedAgentId(agent.id)}
                  className={cn("cursor-pointer", selectedAgentId === agent.id && "bg-primary/10")}
                >
                  <Bot className="size-4 mr-2 opacity-60" />
        ...
        {!selectedAgent ? (
           <div className="flex-1 flex flex-col items-center justify-center text-muted-foreground border-2 border-dashed border-border/50 rounded-2xl bg-muted/10 m-2">
             <Bot className="size-16 mb-4 opacity-20" />
             <p>请在左侧选择一个智能代理以查看详情</p>
           </div>
        ) : (
           <div className="h-full flex flex-col space-y-4 md:space-y-6 animate-in slide-in-from-right-4 duration-300">
             <Card className="p-4 md:p-6 border-border/50 shadow-sm bg-gradient-to-br from-background to-muted/20 relative overflow-hidden">
                ...
                <div className="flex items-start gap-3 md:gap-5 relative z-10">
                  <div className="size-12 md:size-16 rounded-2xl bg-primary flex items-center justify-center text-primary-foreground shadow-xl shadow-primary/20 shrink-0 border border-primary/50">
                    <Bot className="size-8" />
                  </div>
                  <div>
                    <h1 className="text-xl md:text-2xl font-bold tracking-tight">{selectedAgent.identity?.name || selectedAgent.name || selectedAgent.id}</h1>
             ...
             <div className="flex-1 min-h-0 flex flex-col overflow-hidden">
               <Tabs defaultValue="overview" className="flex-1 flex flex-col h-full">
                 <TabsList className="w-full justify-start bg-transparent border-b border-border/50 rounded-none h-12 p-0 px-2 overflow-x-auto flex-nowrap">
                   <TabsTrigger value="overview" className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent data-[state=active]:shadow-none py-3 px-2 font-medium shrink-0">概览</TabsTrigger>
                   <TabsTrigger value="files" className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent data-[state=active]:shadow-none py-3 px-2 font-medium shrink-0">配置</TabsTrigger>
                   <TabsTrigger value="tools" className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent data-[state=active]:shadow-none py-3 px-2 font-medium shrink-0">工具</TabsTrigger>
                   <TabsTrigger value="skills" className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent data-[state=active]:shadow-none py-3 px-2 font-medium shrink-0">技能</TabsTrigger>
                 </TabsList>
                 
                 <TabsContent value="overview" className="flex-1 mt-4 md:mt-6 space-y-4 md:space-y-6 focus-visible:outline-none">
                   <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
                     <Card className="p-4 md:p-5 border-border/50 space-y-4 hover:border-primary/20 transition-colors bg-background/50">
                   ...
                   <Card className="p-4 md:p-6 border-border/50 border-dashed bg-muted/10 text-center text-muted-foreground flex flex-col items-center justify-center min-h-[120px] md:min-h-[150px]">
                     <p className="text-sm">模型选择器 (Model Fallbacks) 面板正在组装</p>
                   </Card>
                 </TabsContent>
                 
                 <TabsContent value="files" className="flex-1 mt-4 md:mt-6 focus-visible:outline-none">
                   <Card className="p-6 md:p-12 border-border/50 border-dashed bg-muted/10 text-center text-muted-foreground flex flex-col items-center justify-center min-h-[200px] md:min-h-[300px]">
                 ...
                 <TabsContent value="tools" className="flex-1 mt-4 md:mt-6 focus-visible:outline-none">
                   <Card className="p-6 md:p-12 border-border/50 border-dashed bg-muted/10 text-center text-muted-foreground flex flex-col items-center justify-center min-h-[200px] md:min-h-[300px]">
                 ...
                 <TabsContent value="skills" className="flex-1 mt-4 md:mt-6 focus-visible:outline-none">
                   <Card className="p-6 md:p-12 border-border/50 border-dashed bg-muted/10 text-center text-muted-foreground flex flex-col items-center justify-center min-h-[200px] md:min-h-[300px]">
