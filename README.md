...
本版本修复了 AI 多工具调用时的流式显示问题：

- **多工具独立卡片**：当 AI 连续调用多个工具时，每个工具的调用和结果现在会显示在独立的卡片中，不再全部堆叠在第一张卡片
- **实时流式更新**：工具调用、思考过程、AI 回复等内容在生成过程中实时分卡显示
- **历史记录整合**：流式结束后自动整合到完整的对话历史中

### 问题修复

- 修复了流式输出时多个 toolResult 全部堆积在一张卡片的问题
- 修复了 WebSocket 连接日志显示不完整的问题
...
当前附件能力走的是**个人服务器临时方案**，仅供试用与功能验证：

- 文件 / 图片 / 音频：通过个人服务器附件接口进行上传与下载
- 视频：由于当前附件接口暂不支持视频上传，视频分享采用外部媒体直链方式渲染

### 重要安全提醒

- 当前传输链路**未做加密处理**
- **请勿传输私密文件、敏感资料、账号密钥、证件信息或其他重要数据**
- 该能力现阶段仅建议用于测试、演示与非敏感内容传输
